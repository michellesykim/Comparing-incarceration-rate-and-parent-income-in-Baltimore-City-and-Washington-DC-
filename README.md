...
#### Python vs. Excel
In terms of data visualizaiton, both Excel and Python generated exact same graphs. However, it was much easier to use Python, since for excel, I had to create a pivot table and then make a pivot chart to make a graph with the average incarceration rate for each parent income percentile whereas in Python, I was just able to use a command to calculate the average and plot the chart. Also for Python, using the zoom function, I was able to zoom in and predict the average incarceration rate for parent income between 25th percentile and 50th percentile or 50th percentile and 75th percentile. 
...
After using both Python and Excel, I would say it was much easier to use Python especially when dealing with a large volume of data. When I did the same project with Excel, I remeber that it took such a long time since Excel was too slow to process a large volume of data. In terms of the result of the analysis/data visualizations, both Python and Excel had similar graphs that have the similar trends. However there was a slight difference. In Excel, for histograms, it gave me the value for each number instead of range (e.g. series point: 0.1, value: 7) whereas in Python, it gave me the value for each range of nubmers (e.g. range: 0.074-0.085, value: 54), so it was much better to know the value that falls into a specific range. Regarding density curves, both Excel and Python had similar trends, but there was a slight difference in terms of the form of the graph. In Excel, since the graph was made of straight lines, the number of neighborhoods was a whole number instaed of decimal points which makes more sense. On the other hand, for Python, since the graph was more curvy and smooth, the number of neighborhoods often contain decimal points (it does not make sense for number of neighborhoods to contain decimal points), which I had a hard time interpreting it. Lastly, for the graph of average incarceration rate, both excel and python generated the same graph. 

For my analyses, both Python and Excel generated similar data visualizations/result, and the only difference was the form (shape) of the graph or the working process to get those graphs. In terms of capability of dealing with large data, I think it is much efficient to use python in my future analysis since it processes data without being lagged. However, in terms of controlling minor details of data or data visualizations, I think it would be much easier to use Excel. For example, if I want to change one part of the data, in Excel, I would just change that part and the graph would automatically reflect that change whereas for Python, I would have to add another line of code or change the data set itself (when the data set is really large and it's hard to preview on Python) and re-upload it. Thus, if both Python and Excel give me the same result (or similar), I think I would mainly use Excel and do parts of analysis that require a large volume of data with python. 
