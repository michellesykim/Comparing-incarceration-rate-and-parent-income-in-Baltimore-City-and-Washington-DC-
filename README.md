...
Histograms/density curve made by Python and Excel are similar to each other. By using Python, it was much easier to deal with a large volume of data without lagging. Specifically for histograms, in excel, I need to set the range of the histogram manually, and if you hover over each columns, it doesn't give the number of counts for a range. For example, if you hover over to the first column, it would give "series point: 0.01, value: 42." On the other hand, in Python, it would give a value for a specific range also if I just set the number of bins, it automatically sets the range for each column. For example, Python would give something like "range: 0.0275-0.0374, count: 18." Thus, when it comes to histogram, Python gives a more specific range for each column. However, I would say it is much easier for excel to format charts especially for people who have no prior experience, also it is much easier to change the data set itself for excel whereas for python, I need to change the data file and then reupload it. For density curve, the data visualization result was somewhat different for Python and Excel. For excel, the density curves were composed of straight lines, which means for each incarceration rate point, it gives a value of whole number like 23. However, for python, the density curve is more of a curvy and smooth graph, which means for every incarceration rate point, there are values that contain decimal points which is not possible for nubmer of neighborhoods. For example, if I hover over one point of the density curve, it would give me value of (0.077,5.622). Thus, in python, for density curves, it is hard to identify the exact number of neighborhoods by just looking at the graph.  
...
Same result as Python vs. Excel for Baltimore City
...
#### Python vs. Excel
In terms of the data visualizaiton, both excel and python give same graphs. However, it was much easier to use Python, since for excel, I had to create a pivot table and then make a pivot chart to make a graph with the average incarceration rate for each parent income percentile whereas in python, I was just able to use a command to calculate the average and plot the chart. 
...
## Python vs. Excel
After using both python and excel, I would say it was much easier to use python especially when dealing with a large volume of data. When I did the same project with excel, I remeber that it took such a long time since excel was too slow to work with a large volume of data. In terms of the result of the analysis/data visualizations, both python and excel had similar graphs that have the similar trends. However there was a slight difference. In excel, for histograms, it gave the value for each number instead of range (e.g. range: 0.1, value: 7) whereas in python, it gave the value for each range of nubmers (e.g. range: 0.074-0.085, value: 54), so it was much better to know the value that falls into a specific range with python. In terms of density curves, both excel and python had a similar trends. However, in excel, since the graph was made of straight lines, the number of neighborhoods was a whole number instaed of decimal points wchich makes more sense. On the other hand, for python, since the graph was more curvy and smooth, the number of neighborhoods often contain decimal points, which I had a hard time interpreting it. Lastly, for the graph of average incarceration rate, both excel and python generated the same graph. 

In my analysis with excel and python, both python and excel generated similar data visualizations/trend, and the only difference was the form (shape) of the graph or the process to get the graphs. In terms of capability of dealing with large data, I think it is much efficient to use python in my future analysis. However, in terms of controlling minor details of data or data visualizations, I think it would be much easier to use excel. Thus, if both python and excel give me the same result (or similar), I think I would mainly use excel and do some parts that require a lot of data processing with python. 
